...
2. **Déclenchement** : La validation se déclenche uniquement si :
   - Le champ a été touché (`touched`) ou modifié (`dirty`)
   - La longueur du mot de passe est d'au moins 8 caractères
3. **Appel API** : Le service `ChangePwdService.verifyOldPassword()` appelle l'endpoint backend `/authentication/verify-password`
4. **État de chargement** : Le signal `isVerifyingOldPassword` affiche un spinner pendant la vérification
5. **Gestion des erreurs** : Le signal `oldPasswordError` stocke le message d'erreur de traduction
...
- [ ] Ajouter un debounce pour éviter trop d'appels API (optionnel)
